# coding: utf-8

"""
    Asterwise API

    Vedic astrology, numerology, horoscope, and PDF report API. Send birth data, get accurate sidereal calculations as structured JSON.  **Base URL:** https://api.asterwise.com  **Authentication:** Bearer token — `Authorization: Bearer YOUR_API_KEY`  Get your free API key at https://asterwise.com

    The version of the OpenAPI document: 2026-03-01
    Contact: support@asterwise.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from asterwise.models.dosha_endpoint_response import DoshaEndpointResponse

class TestDoshaEndpointResponse(unittest.TestCase):
    """DoshaEndpointResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DoshaEndpointResponse:
        """Test DoshaEndpointResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DoshaEndpointResponse`
        """
        model = DoshaEndpointResponse()
        if include_optional:
            return DoshaEndpointResponse(
                doshas = {
                    'key' : asterwise.models.dosha_result.DoshaResult(
                        present = True, 
                        types = [
                            ''
                            ], 
                        details = asterwise.models.details.Details(), 
                        interpretation_summary = '', 
                        keywords = [
                            ''
                            ], 
                        remedies = [
                            ''
                            ], )
                    },
                birth_time_unknown = True,
                fallback_method = ''
            )
        else:
            return DoshaEndpointResponse(
                doshas = {
                    'key' : asterwise.models.dosha_result.DoshaResult(
                        present = True, 
                        types = [
                            ''
                            ], 
                        details = asterwise.models.details.Details(), 
                        interpretation_summary = '', 
                        keywords = [
                            ''
                            ], 
                        remedies = [
                            ''
                            ], )
                    },
        )
        """

    def testDoshaEndpointResponse(self):
        """Test DoshaEndpointResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
