...
class AshtottariResponse(BaseModel):
    """
    AshtottariResponse
    """ # noqa: E501
    root: List[AshtottariPeriod]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in root (list)
        _items = []
        if self.root:
            for _item_root in self.root:
                if _item_root:
                    _items.append(_item_root.to_dict())
            _dict['root'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "root": [AshtottariPeriod.from_dict(_item) for _item in obj["root"]] if obj.get("root") is not None else None
        })
        return _obj
