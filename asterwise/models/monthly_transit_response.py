...
class MonthlyTransitResponse(BaseModel):
    """
    MonthlyTransitResponse
    """ # noqa: E501
    start_date: StrictStr
    end_date: StrictStr
    days: List[DailyTransitResponse]
    peak_aspects: List[TransitAspectSchema]
    __properties: ClassVar[List[str]] = ["start_date", "end_date", "days", "peak_aspects"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.days:
            ...
            _dict['days'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in peak_aspects (list)
        _items = []
        if self.peak_aspects:
            for _item_peak_aspects in self.peak_aspects:
                if _item_peak_aspects:
                    _items.append(_item_peak_aspects.to_dict())
            _dict['peak_aspects'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "days": [DailyTransitResponse.from_dict(_item) for _item in obj["days"]] if obj.get("days") is not None else None,
            "peak_aspects": [TransitAspectSchema.from_dict(_item) for _item in obj["peak_aspects"]] if obj.get("peak_aspects") is not None else None
        })
        return _obj
